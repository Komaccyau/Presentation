...
def get_bot_response(user_input, level):
    # プロンプトを設定
    prompt = (
        f"You are a veteran English teacher from the UK who has been teaching English to Japanese people for 20 years. Please reply to {user_input} in English. Please reply to {user_input} with a topic of conversation that will expand the conversation and make it easier for the user to send the next message. Also, try to keep the conversation to the {level}.The reply format is the content of the reply to {user_input}."
    )
