...
const Chat: React.FC = () => {
  ...
  const fetchBotResponse = async (message: string) => {
    const endpoint = 'http://192.168.3.160:5000/chat'; // バックエンドのチャットエンドポイントを修正
    ...
    try {
      ...
      if (data.reply) {
        return `${data.reply}\n\n返信の返信１: ${data.follow_up_1 || 'フォローアップ情報がありません'}\n返信の返信２: ${data.follow_up_2 || 'フォローアップ情報がありません'}`; // フォーマットされた応答を返す
      } else {
